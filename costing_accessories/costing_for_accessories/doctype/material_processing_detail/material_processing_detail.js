...
frappe.ui.form.on("Material Processing Detail", {
    ...
    after_save(frm) {
        ...
        if (!isNaN(cost_per_meter) && parseFloat(cost_per_meter)>0) {
            frappe.db.set_value('Material Processing', parent_name, 'cost_per_meter', cost_per_meter)
            .then(r => {
                var val = r.message;
                // console.log(val);
                ...
                .then(doc => {
                    ...
                    if (lamination_for=="Fabric" || lamination_for=="Foam") {
                        ...
                        if (!isNaN(cost_per_cup)) {
                            ...
                            frappe.db.set_value('Material Processing', parent_name, 'cost_per_cup', cost_per_cup)
                            .then(r => {
                                var val = r.message;
                                back_to_prev_route(frm);
                            });
