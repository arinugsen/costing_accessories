...
frappe.ui.form.on("Costing Accessories Manual", {
	refresh(frm) {
        ...
        frm.add_custom_button('Go To Costing', () => {
            frappe.db.exists('Costing Accessories', frm.doc.name)
            .then(exists => {
                console.log(exists); // true
                if (exists) {
                    frappe.set_route('Form', "Costing Accessories", frm.doc.name);
                } else {
                    frappe.set_route('Form', "Costing Accessories");
                }
            });
        });
	},
        ...
        .then(r => {
            ...
            frappe.db.get_value('Size', sr2, 'no')
                .then(r => {
                    ...
                    frappe.call({
                        method:
                            "costing_accessories.costing_for_accessories.doctype.costing_accessories_manual.costing_accessories_manual.get_pairs_meter",
                        type: "GET",
                        args: {
                            sr1: sr1,
                            sr2: sr2,
...
// Table Raw Material Manual
frappe.ui.form.on('Raw Material Manual', { 
...
// Table Material Processing Manual
frappe.ui.form.on('Material Processing Manual', { 
    // Triggered when a row is added to a Table field
    ...
    detail(frm, cdt, cdn) {
        ...
        console.log(child);
        var name = row.name;
        // Insert Material Processing Detail Manual if not exist
        frappe.db.exists('Material Processing Detail Manual', name)
        .then(exists => {
            console.log(exists); // true
            if (!exists) {
                frappe.db.insert({
