...
frappe.ui.form.on("Costing Accessories", {
	refresh(frm) {
        ...
        frm.add_custom_button(__('Duplicate Costing Manual'), function(){
            // frappe.msgprint("Duplicate Manual");
            duplicate_manual(frm);
        }, __("Duplicate"));
        frm.add_custom_button(__('Duplicate This Data'), function(){
            // frappe.msgprint("Duplicate Here");
            duplicate_row(frm);
        }, __("Duplicate"));
        ...
        //     duplicate_row(frm);
        // });
        frm.add_custom_button('Go To Costing Manual', () => {
            frappe.db.exists('Costing Accessories Manual', frm.doc.name)
            .then(exists => {
                console.log(exists); // true
                if (exists) {
                    frappe.set_route('Form', "Costing Accessories Manual", frm.doc.name);
                } else {
                    frappe.set_route('Form', "Costing Accessories Manual");
                }
            });
        });
...
function set_reference_doc_name(frm) {
    // Set reference_doc_name every row in table_material_processing
    var row =  frm.doc.table_material_processing;
    // console.log("table_material_processing");
    // console.log(row);
    for (var x=0; x<row.length; x++) {
        frappe.model.set_value(row[x]["doctype"], row[x]["name"], "reference_doc_name", row[x]["name"]);
    }
}
...
function duplicate_row(frm) {
    ...
    // Insert Costing Accessories
    frappe.db.insert({
        doctype: 'Costing Accessories',
...
// Function duplicate manual
function duplicate_manual(frm) {
    var doc_ori = frm.doc;
            ...
            frappe.db.insert({
                ...
                frappe.call('costing_accessories.costing_for_accessories.doctype.costing_accessories.costing_accessories.rename_doc_name', {
                    ...
                    for (var x=0; x<row.length; x++) {
                        ...
                        console.log("reference_doc_name");
                        console.log(reference_doc_name);
                        task_manual(x, reference_doc_name, reference_doc_name_ref);
                    }
                    
                    setTimeout(function() {
                        console.log("Duplicate Manual Done");
                        console.log(doc);
                        msgprint("Duplicate Manual Done");
                    }, 2500 * x);
                });
...
function task_manual(i, reference_doc_name, reference_doc_name_ref) { 
    setTimeout(function() {
        frappe.db.exists('Material Processing Detail', reference_doc_name)
        .then(exists => {
            console.log(exists);
...
frappe.ui.form.on('Material Processing', { 
    ...
    form_render(frm, cdt, cdn) {
        ...
        if (name.substring(0,3) == "new") {
            ...
            frappe.model.set_value(cdt, cdn, "reference_doc_name", name);
